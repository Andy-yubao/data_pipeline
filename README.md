...
本项目为 AutoCar 自动驾驶小车项目的数据处理核心模块。小车在运行时通过**双摄像头**采集驾驶图像——前置摄像头 (FV) 拍摄前方道路，顶视摄像头 (GV) 拍摄俯视地图全景——两者以散装 `.jpg` 文件的形式同时存储在 `datas/` 目录中。数据流水线负责将这些原始数据转化为可供模型直接训练的标准化 **tub 格式**数据集。
...
采集端同时输出两类图像——**前置视图 (FV)** 和 **顶视图 (GV)**，共同存放在 `datas/` 目录下。
...
采集端除前置视图 (FV) 外，还同时输出**顶视/全局视图 (GV)** 图像——从俯视角度观察小车在地图上的位置。GV 图像命名格式：

```
GV_<timestamp>_<COMMAND>_<X>_<Y>.jpg
```
...
| 字段 | 格式 | 示例 | 说明 |
|------|------|------|------|
| `GV` | 固定前缀 | `GV` | 顶视图标识，不可修改 |
| `timestamp` | Unix 毫秒时间戳（13位数字） | `1783244366306` | 采集时刻，与 FV 共用同一时钟 |
| `COMMAND` | 三选一：`FW` / `TR` / `TL` | `FW` | 当前帧对应的驾驶指令，与 FV 同语义 |
| `X` | 4位数字，前导零补齐 | `0143` | 小车在顶视地图上的 X 坐标（像素） |
| `Y` | 4位数字，前导零补齐 | `1001` | 小车在顶视地图上的 Y 坐标（像素） |

**合法文件名示例：**

```
GV_1783244366306_FW_0143_1001.jpg    ✅ 顶视图，直行，坐标(143, 1001)
GV_1783244372231_TL_0132_1202.jpg    ✅ 顶视图，左转，坐标(132, 1202)
```

**GV 图像特性：**

- 分辨率远大于 FV（文件体积约 FV 的 40 倍），为顶视地图的完整截图
- (X, Y) 坐标标记了小车在顶视图中的当前位置，可用于位置监督学习
- GV 和 FV 具有相同的行为标签语义（FW/TL/TR），标签一一对应
...
```
┌──────────────┐
│  FV (259张)  │  ≈70% 精确匹配    │  GV (375张)  │
│  FV-only: 76 │  ← 183对一致 →    │  GV-only: 192│
└──────────────┘                    └──────────────┘
```

| 项目 | 数值 | 说明 |
|------|------|------|
| 精确匹配 | 183 对 | 时间戳完全相同 + 行为标签一致 |
| FV 独有 | 76 张 | 有前置图但无对应顶视图 |
| GV 独有 | 192 张 | 有顶视图但无对应前置图 |
| GV 采样频率 | 比 FV 高约 45% | 顶视图采集间隔更短 |

**关键规则：**

- 当 FV 与 GV 时间戳匹配时，行为标签 (FW/TL/TR) **100% 一致**
- 原始 `datas/` 文件夹中**同时包含 FV 和 GV 两类图片**，数据处理工具会自动区分并配对
- 仅需前置视图的场景：忽略 GV 文件，直接使用 FV 文件即可
...
> `data_processor` 在标准化过程中会使用 `Image.resize((320, 180))` 将所有图像统一缩放至此分辨率。
