...
| GUI 面板 | `data_launcher` | — | 三功能独立入口，各自在二级窗口操作 |
...
```
data_launcher.py  ── GUI 主页面 ──▶ ReviewPage  ──▶ data_reviewer.run_reviewer()
                                  ├──▶ AugmentPage ──▶ data_processor.create_tub()
                                  └──▶ MergePage   ──▶ data_merger.merge_datasets()
                                  各页面独立拥有日志面板 (LogRedirector)
...
操作流程：
1. 在主页面点击 **「数据清洗」** → 选择数据集目录 → 点击「开始审核」→ 在 OpenCV 窗口中逐帧审查，按 `X` 标记垃圾图片，按 `Q` 退出
2. 回到主页面，点击 **「数据增强与均衡」** → 选择数据集目录，调整增强参数和均衡比例 → 点击「标准化处理」→ 自动生成 `tub/` 训练集
3. 如需合并多个数据集，点击 **「数据合并」** → 添加数据源、设置比例和目标目录 → 点击「合并数据集」
...
**功能：** 基于 tkinter 的图形化启动面板，主页面提供三个功能入口，各功能在独立二级窗口中执行。
...
| 页面 | 窗口标题 | 功能 |
|------|---------|------|
| 数据清洗 | `数据清洗 - 人工审核` | 选择数据集/垃圾箱目录 → 开始审核 → OpenCV 逐帧审查 |
| 数据增强与均衡 | `数据增强与均衡` | 选择目录 + 增强参数 + 均衡比例 → 标准化处理（后台线程） |
| 数据合并 | `数据合并` | 多数据源按比例采样 → 合并到目标目录（后台线程） |

**特性：**

- 三个功能完全独立，各自拥有独立的日志面板
- `print` 输出自动重定向到对应页面的日志区（线程安全）
- 重复点击同一入口按钮会激活已有窗口，不会重复创建
- 增强/均衡和合并操作在后台线程执行，界面不阻塞
- 审核清洗时自动隐藏二级窗口，交由 OpenCV 接管；审核结束后恢复
...
由于赛道形状导致驾驶指令分布不均衡（如左转赛道上右转数据极少），`data_processor` 支持在生成 tub 训练集时按指定比例自动均衡各类别的样本数量。

均衡操作**不修改原始 `datas/`**，只控制哪些帧以何种方式进入 `tub/`。

### 两种均衡模式

| 模式 | 行为 | 数据量 | 适用场景 |
|------|------|--------|---------|
| **降采样** (downsample) | 以最紧缺类别为基准，随机丢弃其他类别多余帧 | 总量减少，无重复数据 | 数据总体量充足 |
| **升采样** (upsample) | 以最充裕类别为基准，对不足类别加大增强力度生成变体 | 总量增加，少数类含合成数据 | 数据总体量不足，需扩充 |

### 降采样原理

```
例: FW=1338张, TL=74张, TR=1213张, 目标比例 FW:TL:TR = 2:2:1

base = min(1338/2, 74/2, 1213/1) = min(669, 37, 1213) = 37
目标: FW=37×2=74张, TL=37×2=74张, TR=37×1=37张

结果: 随机采74张FW + 全部74张TL(仅37可采) + 随机采37张TR → 共185张
```

> **注意：** 使用 `--no-augment` 时降采样可达精确比例。若同时启用数据增强，增强变体会加入记录，最终比例略有偏差。
...
base = max(1338/2, 74/2, 1213/1) = 1213
目标: FW=2426张, TL=2426张, TR=1213张

倍率: FW=0.81, TL=31.78, TR=0.00
→ TL 帧的增强概率放大 ~32×，FW 帧放大 ~1.8×，TR 不变
→ 不足类别通过更多增强变体补齐
```

> **注意：** 升采样依赖数据增强生效，需保持至少一种增强开启。极端不均衡时倍率可能很大，导致少数类被反复增强——建议先用降采样，数据充足后再考虑升采样。
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--ratio` | 无（不启用） | 格式 `"FW:N,TL:N,TR:N"`，支持缩写如 `"FW:2,TR:1"` |
| `--balance-mode` | `downsample` | `downsample` 降采样 / `upsample` 升采样 |

### GUI 配置

在 GUI 面板中勾选「启用类别均衡」，设置目标比例和模式，标准化处理时自动生效。
...
1. 将采集图片放入 user/<赛道名>/datas/
   ↓
2. 运行 data_launcher.py
   ↓
3. 点击「数据清洗」，选择对应目录 → 「开始审核」逐帧标记垃圾图片
   ↓
4. 返回主页面，点击「数据增强与均衡」，调整增强参数和均衡比例
   ↓
